...
vi.mock('react', async () => {
  ...
  return {
    ...actual,
    default: { ...actual, useState, useMemo, useCallback, useRef, useEffect },
    useState,
    useMemo,
    useCallback,
    useRef,
...
function renderPage() {
  return renderToStaticMarkup(<PdfToExcelPage />);
}
...
function makeResult(overrides: Partial<PdfToExcelResult> = {}): PdfToExcelResult {
  return {
    download_url: 'https://files.example.com/tables.xlsx',
...
function createPdfFile(name = 'data.pdf', size = 1024, type = 'application/pdf') {
...
describe('PDF to Excel page', () => {
  ...
  describe('actual page render branches', () => {
    ...
    it('renders selected file and convert button (no language radios)', () => {
      reactStateMock.queue = [
        createPdfFile('report.pdf', 1024 * 1024),
        '',
        false,
        null,
        Date.now(),
  ...
  describe('actual event handler callbacks', () => {
    ...
    it('exercises oversized file selection via onChange', () => {
      ...
      expect(reactStateMock.setters[0]).toHaveBeenCalledWith(null);
      expect(reactStateMock.setters[1]).toHaveBeenCalledWith(
        '"large.pdf" terlalu besar (maks 20MB).',
      );
      expect(mockReset).toHaveBeenCalled();
    });
    ...
    it('exercises useEffect tracking branches in plain mode', () => {
      ...
      expect(vi.mocked(track)).toHaveBeenCalledWith(
        'task_completed',
        expect.objectContaining({ tool: 'pdf-to-excel', tables_found: 5 }),
      );
      ...
      expect(vi.mocked(track)).toHaveBeenCalledWith(
        'task_failed',
        expect.objectContaining({ tool: 'pdf-to-excel', error: 'Gagal konversi.' }),
      );
    });
    ...
    it('exercises handleConvert validation-error branch', async () => {
      ...
      const convert = collectElementsByProp(page, 'onClick').find(
        (element) => element.props?.children === 'Konversi ke Excel',
      );
      const click = convert?.props?.onClick;
      if (typeof click === 'function') await click();
      expect(reactStateMock.setters[1]).toHaveBeenCalledWith('"bad.png" bukan file PDF.');
  ...
  describe('fetch, polling, timeout, reset, retry, analytics', () => {
    ...
    it('non-202 submit uses detail/error/status fallback branches', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 422,
        json: async () => ({ error: 'Tabel tidak terbaca.' }),
      });
    ...
    it('polling failed status and polling request failure surface deterministic errors', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ status: 'failed', error: 'Konversi gagal.' }),
      });
    ...
    it('tracks started/completed/failed analytics with pdf-to-excel properties', () => {
      ...
      expect(mockedTrack).toHaveBeenCalledWith(
        'task_started',
        expect.objectContaining({ tool: 'pdf-to-excel' }),
      );
      expect(mockedTrack).toHaveBeenCalledWith(
        'task_completed',
        expect.objectContaining({ tool: 'pdf-to-excel', tables_found: 3 }),
      );
      expect(mockedTrack).toHaveBeenCalledWith(
        'task_failed',
        expect.objectContaining({ tool: 'pdf-to-excel', error: 'Konversi timeout' }),
      );
    });
  });
});
