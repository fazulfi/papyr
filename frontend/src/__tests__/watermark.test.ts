...
describe('STEP-F2-015 — /watermark preview logic coverage', () => {
  ...
  describe('watermark image validation', () => {
    ...
    it('rejects image files above 2MB', () => {
      expect(validateWatermarkImageFile({ type: 'image/png', size: 2 * 1024 * 1024 + 1 })).toBe(
  ...
  describe('watermark image config validation', () => {
    ...
    it('accepts valid image config', () => {
      expect(
        validateWatermarkImageConfig({ opacity: 0.75, position: 'bottom-right', scale: 0.4 }),
      ).toBeNull();
    });
  });
  ...
  describe('preview overlay helpers', () => {
    ...
    it('calculates image overlay positions for all supported placements', () => {
      expect(
        calculateImageOverlayStyle({ opacity: 0.5, position: 'center', scale: 0.4 }, 600, 800),
      ).toEqual({
        x: 300,
        y: 400,
        rotationDegrees: 0,
      ...
      expect(
        calculateImageOverlayStyle({ opacity: 0.5, position: 'top-left', scale: 0.4 }, 600, 800),
      ).toEqual({
        x: 72,
        y: 96,
        rotationDegrees: 0,
      ...
      expect(
        calculateImageOverlayStyle({ opacity: 0.5, position: 'top-right', scale: 0.4 }, 600, 800),
      ).toEqual({
        x: 528,
        y: 96,
        rotationDegrees: 0,
      ...
      expect(
        calculateImageOverlayStyle({ opacity: 0.5, position: 'bottom-left', scale: 0.4 }, 600, 800),
      ).toEqual({
        x: 72,
        y: 704,
        rotationDegrees: 0,
      ...
      expect(
        calculateImageOverlayStyle(
          { opacity: 0.5, position: 'bottom-right', scale: 0.4 },
          600,
          800,
        ),
      ).toEqual({
    ...
    it('derives preview state transitions correctly', () => {
      expect(getPreviewState({ hasPdf: false, isRendering: false, errorMessage: '' })).toBe(
        'placeholder',
      );
      expect(getPreviewState({ hasPdf: true, isRendering: true, errorMessage: '' })).toBe(
        'loading',
      );
      expect(getPreviewState({ hasPdf: true, isRendering: false, errorMessage: 'Gagal' })).toBe(
        'error',
      );
  ...
  describe('STEP-F2-018 image watermark API helpers', () => {
    ...
    it('returns exact error messages for API states', () => {
      ...
      expect(getWatermarkErrorMessage(400, 'Config JSON tidak valid.')).toBe(
