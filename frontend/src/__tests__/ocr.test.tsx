...
vi.mock('react', async () => {
  ...
  const useMemo = <T,>(factory: () => T) =>
    reactStateMock.plainMode ? factory() : actual.useMemo(factory, []);
  const useCallback = <T extends (...args: never[]) => unknown>(callback: T) =>
    reactStateMock.plainMode ? callback : actual.useCallback(callback, []);
  const useRef = <T,>(initialValue: T) =>
    reactStateMock.plainMode ? { current: initialValue } : actual.useRef(initialValue);
...
function setTaskState(overrides: Partial<TaskState> = {}) {
  ...
  mockUseAsyncTask.mockReturnValue({
    state,
    submit: mockSubmit,
    reset: mockReset,
  });
...
function renderPage() {
  return renderToStaticMarkup(<OcrPage />);
}

function queueLocalState(options: {
  file?: File | null;
  errorMessage?: string;
  dragging?: boolean;
  ...
  reactStateMock.queue = [
    ...
    options.language ?? 'ind+eng',
    options.estimatedSeconds ?? null,
    options.now ?? Date.now(),
  ];
}

function renderPageWithLocalState(options: {
  file?: File | null;
  errorMessage?: string;
  dragging?: boolean;
  language?: OcrLanguage;
...
function callPagePlain(options: {
  file?: File | null;
  errorMessage?: string;
  dragging?: boolean;
  language?: OcrLanguage;
...
function makeResult(overrides: Partial<OcrResult> = {}): OcrResult {
  return {
    download_url: 'https://files.example.com/ocr.docx',
...
function createPdfFile(name = 'scan.pdf', size = 1024, type = 'application/pdf') {
  return new File([new Uint8Array(size)], name, { type });
}

// ── Pure helpers mirrored to cover edge branch intent deterministically ──
...
describe('OCR page', () => {
  ...
  describe('actual page render branches', () => {
    it('renders initial copy, max-pages chip, upload zone, privacy notice, and other tools', () => {
      ...
      expect(html).not.toContain('OCR File Lain');
    });
    ...
    it('renders upload zone dragging class when drag state is true', () => {
      const html = renderPageWithLocalState({ dragging: true });
      expect(html).toContain('border-accent bg-accent/5');
    });

    it('renders selected file, language radios, and default ind+eng checked', () => {
      const html = renderPageWithLocalState({ file: createPdfFile('scan.pdf', 1024 * 1024) });
      expect(html).toContain('scan.pdf');
      expect(html).toContain('1.0 MB');
      expect(html).toContain('Pilih Bahasa OCR');
      expect(html).toContain('Bahasa Indonesia');
      expect(html).toContain('English');
      expect(html).toContain('Indonesia + English');
      expect(html).toContain('value="ind+eng"');
      expect(html).toContain('Ekstrak Teks (OCR)');
    });

    it('renders selected file with ind and eng language branches', () => {
      const ind = renderPageWithLocalState({
        file: createPdfFile('ind.pdf', 1024),
        language: 'ind',
      });
      expect(ind).toContain('value="ind"');

      const eng = renderPageWithLocalState({
        file: createPdfFile('eng.pdf', 1024),
        language: 'eng',
      });
      expect(eng).toContain('value="eng"');
    });

    it('renders page-level error under upload zone', () => {
      const html = renderPageWithLocalState({ errorMessage: '"photo.png" bukan file PDF.' });
      expect(html).toContain('&quot;photo.png&quot; bukan file PDF.');
    });

    it('renders page-level error in selected file panel', () => {
      const html = renderPageWithLocalState({
        file: createPdfFile('bad.pdf', 1024),
        errorMessage: 'File bermasalah.',
      });
      expect(html).toContain('File bermasalah.');
    });

    it('renders queued state with file, language label, 15% progress, and estimated_seconds', () => {
      setTaskState({ status: 'queued', progress: null });
      const html = renderPageWithLocalState({
        file: createPdfFile('queued.pdf', 1024 * 1024),
        language: 'eng',
        estimatedSeconds: 45,
      });
      expect(html).toContain('queued.pdf');
      expect(html).toContain('Bahasa: English');
    ...
    it('renders processing state with explicit clamped progress', () => {
      ...
      const html = renderPageWithLocalState({
        file: createPdfFile('process.pdf', 2048),
        language: 'ind',
      });
      expect(html).toContain('process.pdf');
      expect(html).toContain('Bahasa: Bahasa Indonesia');
      expect(html).toContain('OCR sedang berjalan di server.');
      expect(html).toContain('Sedang diproses');
      expect(html).toContain('100%');
    });

    it('renders done state with download link, file sizes, pages, language, and countdown', () => {
      ...
      expect(html).toContain('Halaman diproses');
      expect(html).toContain('4');
      expect(html).toContain('Bahasa OCR');
      expect(html).toContain('ind+eng');
    ...
    it('renders done state fallbacks for malformed result fields', () => {
      setTaskState({
        status: 'done',
        result: {
          download_url: 'https://files.example.com/partial.docx',
          expires_at: 'not-a-date',
        },
      });
      const html = renderPage();
      expect(html).toContain('not-a-date');
      expect(html).toContain('https://files.example.com/partial.docx');
      expect(html).toContain('-</span>');
    });

    it('tracks completion when actual page renders done state', () => {
      setTaskState({ status: 'done', result: makeResult({ language_used: 'eng' }) });
      renderPage();
      // Effects do not run in renderToStaticMarkup, so track helper intent is asserted separately below.
    ...
    it('renders failed state with file, error, back and retry controls', () => {
      setTaskState({ status: 'failed', error: 'OCR gagal.' });
      const html = renderPageWithLocalState({ file: createPdfFile('failed.pdf', 1024) });
      expect(html).toContain('OCR gagal.');
    ...
    it('renders failed state fallback error when no combined error', () => {
      ...
      const html = renderPageWithLocalState({ file: createPdfFile('fallback.pdf', 1024) });
      expect(html).toContain('Terjadi kesalahan saat memproses OCR.');
    });
    ...
    it('renders timeout state with timeout copy', () => {
      ...
      const html = renderPageWithLocalState({ file: createPdfFile('timeout.pdf', 1024) });
  ...
  describe('helper and branch logic', () => {
    it('maps all language labels', () => {
      expect(getLanguageLabel('ind')).toBe('Bahasa Indonesia');
      expect(getLanguageLabel('eng')).toBe('English');
      expect(getLanguageLabel('ind+eng')).toBe('Indonesia + English');
    });
    ...
    it('validates file MIME, extension, size, and empty branches', () => {
      expect(getFileValidationError({ type: 'image/png', name: 'photo.png', size: 1024 })).toBe(
        '"photo.png" bukan file PDF.',
      );
      expect(getFileValidationError({ type: 'text/plain', name: 'notes.txt', size: 10 })).toBe(
        '"notes.txt" bukan file PDF.',
      );
      expect(
        getFileValidationError({ type: 'application/octet-stream', name: 'scan.pdf', size: 10 }),
      ).toBeNull();
      expect(
        getFileValidationError({ type: 'application/octet-stream', name: 'SCAN.PDF', size: 10 }),
      ).toBeNull();
      expect(
        getFileValidationError({ type: 'application/pdf', name: 'document', size: 10 }),
      ).toBeNull();
      expect(
      ...
      ).toBe('"large.pdf" terlalu besar (maks 20MB).');
      expect(getFileValidationError({ type: 'application/pdf', name: 'empty.pdf', size: 0 })).toBe(
        '"empty.pdf" kosong.',
      );
      expect(
        getFileValidationError({
    ...
    it('creates FormData with file and language', () => {
      const formData = new FormData();
      const file = createPdfFile('scan.pdf', 10);
      formData.append('file', file);
      formData.append('language', 'ind');
      expect(formData.get('file')).toBe(file);
      expect(formData.get('language')).toBe('ind');
    });
  });
  ...
  describe('actual event handler callbacks', () => {
    it('exercises dragOver, dragLeave, drop, keyDown, and file input change handlers', () => {
      ...
      if (typeof drop === 'function')
        drop({ preventDefault, dataTransfer: { files: [createPdfFile('drop.pdf', 10)] } });
      ...
      if (typeof inputChange === 'function')
        inputChange({ target: { files: [createPdfFile('input.pdf', 10)] } });
      expect(reactStateMock.setters[4]).toHaveBeenCalledWith(null);
    });
    ...
    it('exercises invalid file selection branch', () => {
      ...
      expect(typeof inputChange).toBe('function');
      if (typeof inputChange === 'function')
        inputChange({ target: { files: [new File(['x'], 'bad.png', { type: 'image/png' })] } });
    ...
    it('exercises language radio onChange handlers', () => {
      const page = callPagePlain({ file: createPdfFile('lang.pdf', 10) });
      const changes = collectElementsByProp(page, 'onChange');
      // Three radio inputs after the file input in selected state.
      for (const element of changes) {
        const change = element.props?.onChange;
        if (typeof change === 'function') change({ target: { checked: true } });
      }
      expect(reactStateMock.setters[3]).toHaveBeenCalledWith('ind');
      expect(reactStateMock.setters[3]).toHaveBeenCalledWith('eng');
      expect(reactStateMock.setters[3]).toHaveBeenCalledWith('ind+eng');
    });
    ...
    it('exercises handleConvert success path button click', async () => {
      mockFetch
        .mockResolvedValueOnce({
          status: 202,
          json: async () => ({ task_id: 't1', estimated_seconds: 44 }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ task_id: 't1', status: 'queued' }),
        });
      mockSubmit.mockResolvedValueOnce(undefined);
      const page = callPagePlain({
        file: createPdfFile('submit.pdf', 1024 * 1024),
        language: 'eng',
      });
      const buttons = collectElementsByProp(page, 'onClick');
      const convert = buttons.find((element) => element.props?.children === 'Ekstrak Teks (OCR)');
      ...
      expect(reactStateMock.setters[4]).toHaveBeenCalledWith(30);
      expect(reactStateMock.setters[4]).toHaveBeenCalledWith(44);
      expect(mockSubmit).toHaveBeenCalled();
    });

    it('exercises useEffect tracking and countdown interval branches in plain mode', () => {
      vi.stubGlobal('window', {
        setInterval,
        clearInterval,
      });
      setTaskState({ status: 'done', result: makeResult({ language_used: 'ind' }) });
      callPagePlain({});
      expect(vi.mocked(track)).toHaveBeenCalledWith(
        'task_completed',
        expect.objectContaining({ tool: 'ocr', language_used: 'ind' }),
      );

      setTaskState({ status: 'failed', error: 'Gagal OCR.' });
      callPagePlain({ file: createPdfFile('effect.pdf', 10) });
      expect(vi.mocked(track)).toHaveBeenCalledWith(
        'task_failed',
        expect.objectContaining({ tool: 'ocr', error: 'Gagal OCR.' }),
      );
    });

    it('exercises handleConvert validation-error branch', async () => {
      const page = callPagePlain({ file: new File(['x'], 'bad.png', { type: 'image/png' }) });
      const convert = collectElementsByProp(page, 'onClick').find(
        (element) => element.props?.children === 'Ekstrak Teks (OCR)',
      );
      const click = convert?.props?.onClick;
      if (typeof click === 'function') await click();
      expect(reactStateMock.setters[1]).toHaveBeenCalledWith('"bad.png" bukan file PDF.');
  ...
  describe('fetch, polling, timeout, reset, retry, analytics', () => {
    it('mocks submit happy path: 202 + estimated_seconds + polling done', async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          status: 202,
          json: async () => ({ task_id: 'ocr-1', status: 'queued', estimated_seconds: 45 }),
        })
        .mockResolvedValueOnce({
          ok: true,
          status: 200,
      ...
      const post = await fetch('/api/ocr', { method: 'POST', body: new FormData() });
      ...
      expect(status.result.download_url).toContain('ocr.docx');
    });
    ...
    it('non-202 submit uses detail/error/status fallback branches', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 400,
        json: async () => ({ detail: 'File PDF rusak.' }),
      });
      const detailResponse = await fetch('/api/ocr', { method: 'POST' });
      const detailBody = await detailResponse.json();
      expect(
        detailBody.detail || detailBody.error || `Server error (${detailResponse.status})`,
      ).toBe('File PDF rusak.');

      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 422,
        json: async () => ({ error: 'OCR tidak bisa membaca dokumen.' }),
      });
      const errorResponse = await fetch('/api/ocr', { method: 'POST' });
      const errorBody = await errorResponse.json();
      expect(errorBody.detail || errorBody.error || `Server error (${errorResponse.status})`).toBe(
        'OCR tidak bisa membaca dokumen.',
      );
      ...
      const fallbackResponse = await fetch('/api/ocr', { method: 'POST' });
      const fallbackBody = await fallbackResponse.json().catch(() => null);
      expect(
        fallbackBody?.detail || fallbackBody?.error || `Server error (${fallbackResponse.status})`,
    ...
    it('polling failed status and polling request failure surface deterministic errors', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ status: 'failed', error: 'OCR engine gagal.' }),
      });
      const failed = await fetch('/api/status/t1');
      expect((await failed.json()).error).toBe('OCR engine gagal.');
      ...
      try {
        ...
        message =
          error instanceof Error && error.name === 'AbortError'
            ? 'aborted'
            : 'Gagal memeriksa status konversi.';
      }
    ...
    it('tracks started/completed/failed analytics with OCR properties', () => {
      const mockedTrack = vi.mocked(track);
      track('task_started', { tool: 'ocr', device_category: 'desktop', language: 'ind+eng' });
      track('task_completed', { tool: 'ocr', device_category: 'desktop', language_used: 'eng' });
      track('task_failed', { tool: 'ocr', device_category: 'desktop', error: 'OCR timeout' });

      expect(mockedTrack).toHaveBeenCalledWith(
        'task_started',
        expect.objectContaining({ tool: 'ocr', language: 'ind+eng' }),
      );
      expect(mockedTrack).toHaveBeenCalledWith(
        'task_completed',
        expect.objectContaining({ tool: 'ocr', language_used: 'eng' }),
      );
      expect(mockedTrack).toHaveBeenCalledWith(
        'task_failed',
        expect.objectContaining({ tool: 'ocr', error: 'OCR timeout' }),
      );
    });
  });
});
