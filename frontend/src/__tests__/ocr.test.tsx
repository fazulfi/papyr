...
describe("OCR page", () => {
  ...
  describe("actual page render branches", () => {
    ...
    it("renders selected file with ind and eng language branches", () => {
      const ind = renderPageWithLocalState({ file: createPdfFile("ind.pdf", 1024), language: "ind" });
      expect(ind).toContain('value="ind"');

      const eng = renderPageWithLocalState({ file: createPdfFile("eng.pdf", 1024), language: "eng" });
    ...
    it("renders page-level error in selected file panel", () => {
      const html = renderPageWithLocalState({ file: createPdfFile("bad.pdf", 1024), errorMessage: "File bermasalah." });
    ...
    it("renders queued state with file, language label, 15% progress, and estimated_seconds", () => {
      ...
      const html = renderPageWithLocalState({ file: createPdfFile("queued.pdf", 1024 * 1024), language: "eng", estimatedSeconds: 45 });
    ...
    it("renders processing state with explicit clamped progress", () => {
      ...
      const html = renderPageWithLocalState({ file: createPdfFile("process.pdf", 2048), language: "ind" });
  ...
  describe("actual event handler callbacks", () => {
    ...
    it("exercises handleConvert success path button click", async () => {
      ...
      const page = callPagePlain({ file: createPdfFile("submit.pdf", 1024 * 1024), language: "eng" });
      ...
      expect(mockFetch).toHaveBeenCalledWith("http://localhost:8000/api/ocr", expect.objectContaining({ method: "POST" }));
    ...
    it("exercises useEffect tracking and countdown interval branches in plain mode", () => {
      ...
      callPagePlain({});
      expect(vi.mocked(track)).toHaveBeenCalledWith("task_completed", expect.objectContaining({ tool: "ocr", language_used: "ind" }));

      setTaskState({ status: "failed", error: "Gagal OCR." });
      callPagePlain({ file: createPdfFile("effect.pdf", 10) });
      expect(vi.mocked(track)).toHaveBeenCalledWith("task_failed", expect.objectContaining({ tool: "ocr", error: "Gagal OCR." }));
    });

    it("exercises handleConvert validation-error branch", async () => {
      const page = callPagePlain({ file: new File(["x"], "bad.png", { type: "image/png" }) });
      const convert = collectElementsByProp(page, "onClick").find((element) => element.props?.children === "Ekstrak Teks (OCR)");
      const click = convert?.props?.onClick;
      if (typeof click === "function") await click();
      expect(reactStateMock.setters[1]).toHaveBeenCalledWith('"bad.png" bukan file PDF.');
  ...
  describe("fetch, polling, timeout, reset, retry, analytics", () => {
    ...
    it("non-202 submit uses detail/error/status fallback branches", async () => {
      ...
      mockFetch.mockResolvedValueOnce({ ok: false, status: 422, json: async () => ({ error: "OCR tidak bisa membaca dokumen." }) });
      const errorResponse = await fetch("/api/ocr", { method: "POST" });
      const errorBody = await errorResponse.json();
      expect(errorBody.detail || errorBody.error || `Server error (${errorResponse.status})`).toBe("OCR tidak bisa membaca dokumen.");
    ...
    it("polling failed status and polling request failure surface deterministic errors", async () => {
      mockFetch.mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ status: "failed", error: "OCR engine gagal." }) });
    ...
    it("tracks started/completed/failed analytics with OCR properties", () => {
      ...
      expect(mockedTrack).toHaveBeenCalledWith("task_started", expect.objectContaining({ tool: "ocr", language: "ind+eng" }));
      expect(mockedTrack).toHaveBeenCalledWith("task_completed", expect.objectContaining({ tool: "ocr", language_used: "eng" }));
      expect(mockedTrack).toHaveBeenCalledWith("task_failed", expect.objectContaining({ tool: "ocr", error: "OCR timeout" }));
    });
  });
});
