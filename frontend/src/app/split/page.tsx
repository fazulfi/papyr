...
export default function SplitPage() {
  ...
  const handleFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Gagal membaca file PDF.');
      setSplitState('error');
    }
  }, []);
  ...
  const canSplit = splitState === 'ready' && selectedPages.length > 0 && !rangeError;
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-4 py-8 sm:py-12">
      ...
      <div className="mb-8 text-center flex flex-col items-center">
        ...
        <p className="text-base text-slate-500">Ambil halaman tertentu dari dokumen PDF.</p>
        <p className="mt-2 text-sm text-slate-400 max-w-md">
          Ambil halaman yang kamu butuhkan dari laporan, skripsi, atau e-book tanpa perlu download
          ulang seluruh file.
        </p>
      </div>
      ...
      {splitState === 'done' && splitData && (
        <div className="animate-fade-up rounded-2xl border border-accent/20 bg-white p-6 shadow-[0_4px_20px_rgba(37,99,235,0.06)]">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-semibold text-navy">PDF berhasil dipisahkan!</p>
      ...
      {splitState === 'loading' && (
        <div className="animate-fade-up rounded-2xl border border-slate-200 bg-white p-6">
          <p className="mb-2.5 text-sm font-medium text-slate-500">Membaca dokumen PDF...</p>
          <div className="relative h-1.5 overflow-hidden rounded-full bg-slate-100">
            <div className="absolute inset-0 animate-shimmer rounded-full bg-gradient-to-r from-transparent via-accent to-transparent bg-[length:200%_100%]" />
          </div>
        </div>
      ...
      {splitState === 'ready' && file && (
        <div className="animate-fade-up space-y-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            <PageRangeInput totalPages={totalPages} onChange={handleRangeChange} />
          </div>
      ...
      {splitState === 'idle' && (
        <div className="animate-fade-up">
          ...
          >
            ...
            <p className="text-xs text-slate-400">Maks {limits.maxUploadMB}MB · Hanya file PDF</p>
          </div>
